...
public class Variable {
	...
	public static void main(String[] args) {
		 ...
		 * - 실수 : float, double(더 정확한 숫자를 표기할떄)
		 * - 문자 : char
		 * - 논리 : boolean
		 * 
...
//		System.out.println(_byte); //컴파일 에러 발생 (문법이 틀렸다.)
		...
		//위에서 초기화한 변수에 새로운 값을 저장하고 출력해 주세요
		...
		_long = 800L; 
		_float = 3.24f; 
		_double =2.14; //double 접미사 : d(생략가능)
		_char ='바'; //하나의 문자만 표현이 가능
		_boolean =false;
		...
		//전역변수 : 매서드 밖에 저장되어 있는 값, 초기화 하지 않아도 기본값으로 저장
		//지역변수 : 매서드 안에 저장되어 있는 값
		
		
		
		//double 타입에서 int 타입으로 형변환해서 출력 해주세요
		...
		//표현범위가 작은 타입에서 큰 타입으로 형변환 할 셩우 데이터가 손실될 염려가 없다.
		//그러므로 형변환을 생략할 수 있다.
		_int = _byte;
		_long = _int;
		_double = _int;
		 ...
		 * -final int MAX_NUMBER; //상수는 대문자를 사용한다.

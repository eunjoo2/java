...
public class InnerClass {
	...
	public static void main(String[] args) {
		 ...
		 * -인스턴스 클래스 : 멤버변수 선언위치에 선언하며, 
		 * 					인스턴스 멤버들과 관련된 작업에 사용된다.
		 * -스태틱 클래스 : static 이 붙고 멤버변수 선언위치에 선언하며, 
		 * 					static멤버들과 관젼된 작업에 사용된다.
		 * -지역클래스 : 메서드나 초기화 블럭 안에 선언하며, 
		 * 					선언된 영역 내부에서만 사용될 수 있다.
		 * -익명 클래스 : 이름이 없고, 선언과 객체생성을 동시에 한다.(일회용)
		 */
...
class Outer{
	...
	class InstanceInner{
		...
		void methid(){
			...
			System.out.println(Outer.this.Var); //외부 인스턴스 변수
		}
	}
...
//		int inner1 = instanceVar;
	...
	void method(){
		...
		//지역클래스
		class LocalInner{
			//지역클래스에서는 인스턴스 멤버와 클래스 멤버 모두 사용 가능 하고
...
//			int inner3 = localVar1; //final이 붙지 않은 지역변수는 사용 불가
		...
		//스태틱 클래스는 클래스를 통해서 바로 접근할수 있다.
		System.out.println(StaticInner.inner3);
		
		//스태틱클래스의 인스턴스 멤버에 접근하기 위해서는 인스턴스화를 해야한다.

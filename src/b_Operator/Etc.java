...
public class Etc {
	...
	public static void main(String[] args) {
		 ...
		 * - &(AND)			: 피연산자 양 쪽이 모두 1이면 1을 그외는 0을 결과로 얻느다.
		 * - ^(XOR)			: 피연산자의 값이 서로 다를 때만 1을 같을떄는 0을 결과로 얻는다.
		 * - ~(비트전환)		: 피연산자를 0은 1로 1은 0으로 바꾼다
		 * - <<(쉬프트)		: 피연산자의 각 자리를 왼쪽으로 이동시킨다. 빈칸은 0으로 채운다.
		 * - >>(쉬프트)		: 피연산자의 각 자리를 외쪽으로 이동시킨다. 빈칸을 앞자리의 동일한 값으로 채운다
		 * 
		 * <<기타 연산자>>
		 * 
		 * - .(참조연산자) 	: 특정 볌위내에 속해 있는 멤버를 지칭할 때 사용한다.
		 * - (type)			: 형변환(casting)
		 * - ?:(삼향연산자) 	: 조건식 ? true true/ false false --> 조건식이 참일 경우 수행할 문장 : 조건식이 거짓일 경우 수행할 때 문장
		 * - instanceof		: 참조형타입 확인 
		 * 
		 */
		...
		//변수에 저장된 주민등록번호 뒷자리의 첫번째 숫자가 1이나 3이면 남자를 2나4면 여자를 출력해주세요
		//그 외의 숫자는 확인 불가
		regNo = 1;
		
		gender = regNo == 1 ? "남자" : (regNo == 2 ? "여자" : regNo == 3 ? "남자" : regNo == 4 ? "여자" : "확인불가");
		System.out.println("당신의 성별은 " + gender + " 입니다.");

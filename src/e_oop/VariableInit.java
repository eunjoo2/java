...
public class VariableInit {
		 ...
		 * << 생성자>>
		 * - 클래스와 같은 이름의 메서드
		 * - 객체를 생성하면서 팔요한 작업이 있을때 사용한다. ( 인스턴스 변수 초기화)
		...
		VariableInit(int var){
			for(int i = 1; i <= var; i++){
				this.var += i; //this가 붙는건 인스턴스 변수
			}
			
			//this : 클래스의 인스턴스 주소가 저장된 변수
			//인스턴스 변수와 지역변수의 이름이 같을 때 둘을 구분하기 위해 인스턴스 변수앞에 붙여준다
...
//			}
			...
			//this() 이전의 코드가 의미없는 코드가 될 수 있기 때문에 this()는 생성자의 첫줄에서만 사용할 수 있다.

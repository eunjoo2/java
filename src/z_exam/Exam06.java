...
/*		[6-10] 다음 중 생성자에 대한 설명으로 옳지 않은 것은? (모두 고르시오) b.e
				...
				b. 생성자는 객체를 생성하기 위한 것이다.// new 연산자가 객체생성을 합니다.
				c. 클래스에는 생성자가 반드시 하나 이상 있어야 한다.
				d. 생성자가 없는 클래스는 컴파일러가 기본 생성자를 추가한다.
				e. 생성자는 오버로딩 할 수 없다. // 오버로딩을 
				
				b. 생성자는 객체를 생성하기 위한 것이다.
				→ 생성자가 객체를 생성할 때 사용되기는 하지만, 객체를 초기화할 목적으로 사용되는것이다. 객체를 생성하는 것은 new연산자이다.
				e. 생성자는 오버로딩 할 수 없다.
		...
		[6-13] 다음 중 아래의 add메서드를 올바르게 오버로딩 한 것은? (모두 고르시오) b.c.d
				...
				모두 메서드의 이름이 add이고 매개변수의 타입이 다르므로 오버로딩이 성립한다. 
...
/*		[6-18] 다음의 코드를 컴파일하면 에러가 발생한다. 컴파일 에러가 발생하는 라인과 그이유를 설명하시오.
		...
		int iv2 = cv;
				...
				int cv2 = iv; // 라인 A 컴파일 에러 static변수를 또 다시 초기화 할수 없다. 
		static void staticMethod1() {
		
			System.out.println(cv);
				System.out.println(iv); // 라인 B 컴파일 에러 static메서드는 인스턴스 변수를 사용할 수 없다.
		}
		...
		static void staticMethod2() {
			staticMethod1();
				instanceMethod1(); // 라인 D 컴파일 에러 static메서드는 인스턴스 메서드를 사용할 수 없다.
		}
...
/*		[6-19] 다음 코드의 실행 결과를 예측하여 적으시오.
		...
		String str = "ABC123"; //주소 저장
		System.out.println(str); //ABC123
		change(str);			//파라미터 상단의 주소를 저장
	...
	*/	
		...
		//After change:ABC123
		/*
		 *main 메서드의 출력값은 ABC123 이고 change의 출력값은 ABC123456이다.
		 *string 타입은 새로운 값을 저장해 주면 객체를 생성한다.
		 *
		 *change는 문자열로 바뀌고 주소값만 저장이 된다 이제 change의 메서드가 종료되고 작업에 사용되던 메모리가 제거 되고
		 *참조변수가 사라지면서 ABC123456의 주소값도 사라지게 된다.....?
		 */

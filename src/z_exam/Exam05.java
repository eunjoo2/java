...
public class Exam05 {
	...
	public static void main(String[] args) {
		 ...
		 *  b. int[] arr = {1,2,3,}; 
		 *  c. int[] arr = new int[5]; 
		 *  d. int[] arr = new int[5]{1,2,3,4,5};	[5]를 선언해서는 안됨 
		 *  e. int arr[5]; //배열을 선언할떄 배열안에 크기를 선언할수 없음
		 *  f. int[] arr[] = new int[3][]; -
		 *  
		 *  d.e
...
//			 ballArr 3 ball3 배열 의 앞에서 개의 수를 배열 로 복사한다
			 ...
			 //변수  money의 금액을 동전으로 바꾸었을 때 각각 몇 개의 동전이 필요한지 계산해서 출력하 라
...
//			1.금액(money)을 동전 단위(coinUnit)로 나눠서 필요한 동전의 개수(coinNum)를 구한다 
...
//			3.금액에서 동전의 개수(coinNum)와 동전단위를 곱한 값을 뺀다.
		...
		/*	 //[5-8]
			 ...
			 for(int i=0; i < answer.length;i++) { 
				 counter[answer[i]-1]++; // counter의 배열안에 숫자를 넣을때 -1 필요! 배열은 0부터 시작하니깐
				 } 
			 
			 for(int i=0; i < counter.length;i++) { 
				System.out.print(counter[i]);	//숫자를 하나씩 계산을 하고
				for(int j=0; j<counter[i]; j++){
					System.out.print("*");	//계산한 숫자를 별로 찍어 
...
/*
		...
		for(int i=0;i<words.length;i++) {
			...
			for(int j=0; j<question.length*28; j++){

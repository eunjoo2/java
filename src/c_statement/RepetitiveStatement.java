...
public class RepetitiveStatement {
	...
	public static void main(String[] args) {
		...
		sum = 0;
		//반복문을 사용하면 규칙이 있는 반복되는 작업을 짦게 표현할 수 있다.
		for(int i = 1; i <= 10; i++){
			sum += i;
		}
		System.out.println(sum);
		...
		sum = 0;
		for(int i = 100; i >= 1; i--){//100부터 1까지 1씩 감소하면서 반속
			sum += i;
		}
		System.out.println(sum);
...
//		}
		...
		outer : for(int i = 2; i <= 9; i++){
			for(int j =1; j <= 9; j++){
				if (j == 4){
//				break; //가장가까운 반복문 하나를 빠져나간다.
//					break outer; // outer라는 이름의반복문을 빠져나간다.
//					continue;//가장 가까운 반복문의 현재 반복회자를 빠져나간다.
					continue outer;// outer라는 이름의 현재 반복회자를 빠져나간다.
				}
